...
from modules.delta_optimizer import DeltaOptimizer
...
class Toy(nn.Module):
    ...
    def forward(self, x, compute_grad=False):
        ...
        # f1 = ((0.5*(-x1-7)-torch.tanh(-x2)).abs()+SMALL).log() + 6
        # f2 = ((0.5*(-x1+3)+torch.tanh(-x2)+2).abs()+SMALL).log() + 6
        ...
        if compute_grad:
            ...
            g11_tensor = torch.autograd.grad(f1, x1, retain_graph=True)[0]
            g12_tensor = torch.autograd.grad(f1, x2, retain_graph=True)[0]
            g21_tensor = torch.autograd.grad(f2, x1, retain_graph=True)[0]
            g22_tensor = torch.autograd.grad(f2, x2)[0]

            g = torch.stack([
                torch.stack([g11_tensor, g21_tensor]),
                torch.stack([g12_tensor, g22_tensor])
            ])
...
def plot3d(F, xl=11):
    ...
    # 修改后（正确）：
    for label in ax.xaxis.get_ticklabels():  # 获取刻度标签（Text对象）
        label.set_fontsize(15)  # Text对象有fontsize属性
...
def run_all():
    ...
    inits = [
        torch.tensor([-8.5, 7.5], dtype=torch.float64),
        torch.tensor([-8.5, -5.], dtype=torch.float64),
        torch.tensor([9.,   9.], dtype=torch.float64),
    ]
    ...
    for i, init in enumerate(inits):
        for m in tqdm(["sgd", "mgd", "pcgrad", "cagrad"]):
            ...
            all_grads[m] = None
            all_fixed_grads[m] = None

            all_params_delta[m] = None
            ...
            n_iter = 100000
            opt = DeltaOptimizer(torch.optim.Adam([x], lr=0.001))

            for it in range(n_iter):
                traj.append(x.detach().numpy().copy())

                f, grads = F(x, True)
                grads_record.append(grads.detach().numpy().copy())
                if m== "cagrad":
                ...
                opt.zero_grad()
                x.grad = g
                fixed_grads_record.append(g.detach().numpy().copy())
                params_delta = opt.step()

                params_delta_flatten = torch.cat([param_delta.flatten() for param_delta in params_delta],dim=0)
                param_delta_record.append(params_delta_flatten.detach().numpy().copy())

            all_traj[m] = torch.tensor(np.array(traj))
            all_grads[m] = torch.tensor(np.array(grads_record))
            all_fixed_grads[m] = torch.tensor(np.array(fixed_grads_record))
            all_params_delta[m] = torch.tensor(np.array(param_delta_record))

        os.makedirs(f"{RESULTS_FOLDER}\\{PROBLEM_NAME}", exist_ok=True)
        torch.save(all_traj, f"{RESULTS_FOLDER}\\{PROBLEM_NAME}\\toy{i}.pt")
        torch.save(all_grads, f"{RESULTS_FOLDER}\\{PROBLEM_NAME}\\toy_all_task_grads{i}.pt")
        torch.save(all_fixed_grads, f"{RESULTS_FOLDER}\\{PROBLEM_NAME}\\toy_all_task_fixer_grads{i}.pt")
        torch.save(all_params_delta, f"{RESULTS_FOLDER}\\{PROBLEM_NAME}\\param_delta{i}.pt")
